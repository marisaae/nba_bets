...
# # Fetch team info
lal_team_info = teaminfocommon.TeamInfoCommon(team_id=lal_team_id)
team_df = lal_team_info.get_data_frames()[0]
...
if not team_df.empty:
    ...
    team_record = str(team_df.loc[0, 'W']) + "-" + str(team_df.loc[0, 'L'])
    ...
    with psycopg.connect(dsn) as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO team_info (id, team_name, abbreviation, location, conference, record, total_win, total_loss, standing)
...
# # Fetch team roster
lal_roster = commonteamroster.CommonTeamRoster(team_id=lal_team_id)
roster_df = lal_roster.get_data_frames()[0]
...
# # Fetch team schedule
league_schedule = scheduleleaguev2.ScheduleLeagueV2(season='2025-26')
schedule_df = league_schedule.get_data_frames()[0]
lakers_df = schedule_df[(schedule_df['homeTeam_teamId'] == 1610612747) | (schedule_df['awayTeam_teamId'] == 1610612747)]
columns = [
    'gameId',
    'gameLabel',
    'seasonYear',
...
lakers_schedule = lakers_df[columns].reset_index(drop=True)

if not lakers_schedule.empty:
    with psycopg.connect(dsn) as conn:
        with conn.cursor() as cur:
            for _, row in lakers_schedule.iterrows():
                game_id = row['gameId']
...
for _, row in roster_df.iterrows():
    player_id = row['PLAYER_ID']
    try:
        player_log = playergamelog.PlayerGameLog(player_id=player_id, season='2025-26')
